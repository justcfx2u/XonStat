...
// Draw the accuracy chart in the "chartArea" div id
...
// Calculate the frags Y value for a given weapon stat
...
// Calculate the frags tooltip text for a given weapon stat
...
// Draw the frag chart into the "chartArea" div id
function drawFragChart(weaponData) {
  ...
  for (i in weaponData.games) {
    ...
    for (w in weaponColors) {
      var pct = fragValue(w, ws) / totalFrags;
  ...
  var options = getPercentageOptions(true);
...
function drawDamageChart(weaponData) {
  ...
  var options = getDamageOptions(true);
  options.legend.maxLines = 3;
  options.isStacked = true;
